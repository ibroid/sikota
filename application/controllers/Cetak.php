...
class Cetak extends CI_Controller
{
	...
	private static function perkara_pihak_replace($data)
	{
		return [
			...
			'nama_pe' =>  sippTable()::lawanPihak($data['nomor_perkara'], 'pihak1_text'),
			'jenis_pihak_pe' =>  sippTable()->whoIsIt($data['nomor_perkara'], 'P'),
			'nama_te_1' =>  sippTable()::lawanPihak($data['nomor_perkara'], 'pihak2_text'),
			'jenis_pihak_te' =>   sippTable()->whoIsIt($data['nomor_perkara'], 'T'),
	...
	public function cetak_pengantar_keluar($id)
	{
		...
		switch ($data['jenis_delegasi_text']) {
			case 'Pemberitahuan Putusan Pengadilan Tingkat Pertama':
				$this->pbt_tingkat_pertama($data);
				break;
			case 'Panggilan Sidang':
				$this->panggilan_sidang($data);
				break;
		}
	}
	public function pbt_tingkat_pertama($data)
	{
		...
		$file = Export::findFile("./rtf/template/template_pengantar_pbt.rtf")
			->replace($export)->write("./rtf/hasil/hasil_pbt_keluar.rtf");
	...
	{
		$data = Tabayun_keluar::getWhere(['id' => $id])->row_array();
		$file = Export::findFile("./rtf/template/template_amplop_pengantar.rtf")
			->replace(array_merge([
				'nomor_perkara' => $data['nomor_perkara'],
